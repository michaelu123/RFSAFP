...
let eigenesIndex: number; // Ich komme mit meinem eigenen Fahrrad
...
function attachmentFiles(): GoogleAppsScript.Drive.File[] {
  ...
  let attachmentFolder = grandPa.getFoldersByName("Anhänge für Fahrpraxis für Anfänger").next();
...
function anmeldebestätigung() {
  ...
  let zahlungsText =
    "Wir ziehen die Teilnahmegebühr von " + betrag + "€ in den nächsten Tagen ein.";
  ...
  let termin =
    any2Str(kursRow[tag1Index - 1], "E 'den' dd.MM", false) + " von " + kursRow[uhrZeitIndex - 1];
...
function sendeAntwort(
  ...
  template.verifLink =
    "https://docs.google.com/forms/d/e/1FAIpQLSdJzwKx5B_PzR3MHHpzIuNhSzMTP8u3NPe1OndzmZzZe9VHcQ/viewform?usp=pp_url&entry.1730791681=Ja&entry.1561755994=" +
...
function updateForm() {
  ...
  for (let kursObj of kurseObjs) {
    ...
    let desc =
      bolderizeWord(mr) + ", " + kursObj["Uhrzeit"] + ", " + any2Str(kursObj["Tag 1"]) + freiText;
...
function sendWrongIbanEmail(anrede: string, empfaenger: string, iban: string) {
  ...
  var body =
    anrede +
    ",\nDie von Ihnen bei der Buchung von ADFC Fahrpraxiskursen übermittelte IBAN " +
    iban +
...
function printKursMembers() {
  ...
  // first row of temp sheet: the headers
  {
    ...
    row.push(rowValues[tag1Index - 1]);
    sheet.appendRow(row);
  }
